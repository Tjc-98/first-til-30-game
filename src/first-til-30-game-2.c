...
#define COMPUTER_FIRST_POINT_CHOICE 10
...
int processValidPlayersName(char userStringInput[], struct Player gamePlayers[], int playerCount) {
    ...
    struct Player computerPlayer = {"Dator", 0};

    int lastPlaceInPlayerList = playerCount;

    gamePlayers[lastPlaceInPlayerList] = computerPlayer;
...
int processValidPlayersPoint(char userStringInput[], int inputScore[], int playerCount) {
    ...
    // First Round
    if (inputScore[0] == 0) {
        inputScore[lastPlaceInPlayerList] = COMPUTER_FIRST_POINT_CHOICE;
    }
...
int main() {
    ...
    struct Player gamePlayers[MAX_PLAYER_COUNT + 1];
    int inputScore[MAX_PLAYER_COUNT + 1] = {0};
